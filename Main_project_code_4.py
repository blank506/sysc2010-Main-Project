...
def load_csv():
    ...
    # LIMIT to actual duration/might work?
    time = time - time[0]
    ...
    if "ecg" in df.columns:
        ...
        sigcol = df.select_dtypes(include=[np.number]).columns[0]
...
def fir_filter(data, cutoff, fs, btype):
    ...
    if btype == "low":
        ...
        taps = firwin(101, cutoff/nyq, pass_zero=False)
    else:
        taps = firwin(101, [cutoff[0]/nyq, cutoff[1]/nyq], pass_zero=False)
...
def plot_time():
    ...
    if stype == "Temperature":
        duration = 3600 * 24 * 2   # show 2 days
    else:
